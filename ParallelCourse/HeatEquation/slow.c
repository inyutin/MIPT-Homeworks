...
int main(int argc, char *argv[]) {
    ...
    for (int t = 0; t < iterations; t++) {
        if (rank != 0) {
            MPI_Send(&final[width], width, MPI_FLOAT, rank - 1, rank - 1, MPI_COMM_WORLD);
        }
        if (rank != P - 1) {
            MPI_Send(&final[(height - 2)*width], width, MPI_FLOAT, rank + 1, rank + 1, MPI_COMM_WORLD);
        }
    ...
    if (rank != 0) {
        MPI_Send(&final[0], height * width, MPI_FLOAT, 0, rank, MPI_COMM_WORLD);
    }
